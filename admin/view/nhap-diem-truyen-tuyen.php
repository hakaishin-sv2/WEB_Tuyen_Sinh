                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-md-6">
                                            <div class="form-group">
                                                ...
                                                <input readonly type="text" class="form-control" id="industry_code" name="industry_code" placeholder="Nhập mã ngành" value="<?= htmlspecialchars($list[0]['industry_code']); ?>" required>
                                            </div>
                                            <div class="form-group">
                                                <label for="name">Tên ngành:</label>
                                                <input readonly type="text" class="form-control" id="name" name="name" placeholder="Nhập tên ngành" value="<?= htmlspecialchars($list[0]['ten_nganh']); ?>" required>
                                            </div>
                                            <?php foreach ($khoiXetTuyenArray as $khoi): ?>
                                                <div class="form-group">
                                                    <label for="diem_<?= htmlspecialchars($khoi); ?>"><?= htmlspecialchars($khoi); ?></label>
                                                    <?php
                                                    // Lấy điểm cho khối xét tuyển hiện tại từ mảng diemTrungTuyen
                                                    $diem = isset($diemTrungTuyen[$khoi]) ? htmlspecialchars($diemTrungTuyen[$khoi]) : '';
                                                    ?>
                                                    <input type="text" class="form-control" id="diem_<?= htmlspecialchars($khoi); ?>" name="diem[<?= htmlspecialchars($khoi); ?>]" placeholder="Nhập điểm cho <?= htmlspecialchars($khoi); ?>" value="<?= $diem; ?>">
                                                </div>
                                            <?php endforeach; ?>
                                            <!-- Nút submit -->
                                            <div class="form-group">
                                                <button type="submit" class="btn btn-primary">Cập nhật điểm trúng tuyển</button>
                                            </div>
                                        </div>
                                    </div>
                                </form>
